...
if (displ==1)
     ...
     if ~isempty(twod)
         figure;
         hold on;
         plot(Y.coords{twod}(1,:), Y.coords{twod}(2,:), 'bo'); 
         %Y.mypath=mypath;
         if (overlay == 1)
             gplot(E(Y.index, Y.index), [Y.coords{twod}(1,:); Y.coords{twod}(2,:)]','b'); 
             %gplot(E(Y.mypath, Y.mypath), [Y.coords{twod}(1,:); Y.coords{twod}(2,:)]','r');
